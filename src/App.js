...
function App() {
  ...
  const getBNBBalance = async (address) => {
    let balance = 0;
    try {
      const res = await axios.get(`https://deep-index.moralis.io/api/v2/${address}/balance?chain=0xa86a`, {
        headers: {
  ...
  const getBalance = async (contract_address, address) => {
    ...
    try {
      const res = await axios.get(
        `https://api.snowtrace.io/api?module=account&action=tokenbalance&contractaddress=${contract_address}&address=${address}&tag=latest&apikey=${apiKey}`,
      );
     // `https://api.snowtrace.io/api?module=account&action=tokenbalance&contractaddress=${contract_address}&address=${address}&tag=latest&apikey=${apiKey}`
  ...
  const getTokenPriceData = async (symbol = "XPANZ") => {
    let resp;
    try {
  ...
  const getReflected = useCallback(async () => {
    ...
    if (account) {
      try {
        let txndata = await axios.get("https://api.snowtrace.io/api", {
          timeout: 30000,
          params: {
            module: "account",
            action: "tokentx",
            address: account,
            contractaddress: config.XPANZ[config.chainID],
            startblock: 0,
            endblock: 99999999,
            sort: "desc",
        ...
        const movements = txndata.data.result.reduce(
          (acc, data) => {
            if (data.contractAddress !== config.XPANZ[config.chainID].toLowerCase()) {
              return acc;
            }
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const contract = new web3.eth.Contract(ABI, config.XPANZ[config.chainID]);
        ...
        const XPANZAmountofRFV =
          await getBalance(config.XPANZ[config.chainID], config.RFV[config.chainID]);
        const bnbAmountofRFV =
          await getBNBBalance(config.RFV[config.chainID]);
        const treasuryRFV = XPANZAmountofRFV * marketPrice + bnbAmountofRFV * wbnbPrice.usd;
        const pastRFV = 499301.52;

        const XPANZAmountofTreasury =
          await getBalance(config.XPANZ[config.chainID], config.Treasury[config.chainID]);
        const bnbAmountofTreasury =
          await getBNBBalance(config.Treasury[config.chainID]);

        const treasury = treasuryRFV + XPANZAmountofTreasury * marketPrice + bnbAmountofTreasury * wbnbPrice.usd;
