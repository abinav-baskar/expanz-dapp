...
import "./style.css";
import { FaTelegram, FaTwitter, FaMedium, FaDiscord, FaYoutube } from "react-icons/fa";
...
const Sidebar = ({ show, setShow , showAccountAddress}) => {
  return (
    <div className={`sidebar ${show && "active"}  flex flex-col`}>
      <div className="grid grid-flow-col gap-x-3 justify-start  items-center sidebar-logo px-5">
        <Link to="../">
          <img src={Logo_app} alt="" className="px-6" />
        </Link>
      </div>
      <div class="wallet-link" style={{'textAlign':'center', 'marginTop':'-50px', 'fontSize':'12px'}}>
        <a class="text" style={{'fontSize':'1rem'}} href="https://traderjoexyz.com/address/0x49Eda65e43B5167ff9c3982E1DFe2E3DB55C2928"
        target="_blank">{showAccountAddress}</a>
      </div>
      <div className="flex flex-col  justify-between flex-1 px-6 py-4" style={{'marginTop':'50px'}}>
        <ul className=" pr-14">
          {menuList.map((val, i) => (
              ...
              <li>
                {i <= 2 ? (
                  ...
                  <a
                    href={val.link}
                    target={"_blank"}
                    className=" grid grid-flow-col font-medium text-base mb-2 justify-start gap-x-2 items-center py-2 px-6 hover:bg-primary-400 rounded-lg my-1"
                    rel="noreferrer"
                  >
                    { <span className="text-lg">{val.icon}</span> }
                    <span>{val.text}</span>
...
const menuList = [
  ...
  },
  {
    ...
    link: "https://traderjoexyz.com/trade?outputCurrency=0x235Dc4738290Aad823b8C4Ac2012C188C5232933",
  },
  {
    text: "Chart",
    icon: <ChartIcon />,
    link: "https://dexscreener.com/avalanche/0x235Dc4738290Aad823b8C4Ac2012C188C5232933",
  },
  {
    text: "Official site",
    icon: <LibraryBooksIcon />,
    link: "https://expanz.org/",
  },
];

const socialList = [
  {
    link: "https://twitter.com/ExpanzOrg",
    icon: <FaTwitter />,
  },
  {
    link: "https://t.me/ExpanzProtocol",
    icon: <FaTelegram />,
  },
  {
    link: "https://discord.gg/Wm6dGjjrDC",
    icon: <FaDiscord />,
  },
  {
    link: "https://www.youtube.com/channel/UC0XmIx63vrLdIvugMDLrDlQ",
    icon: <FaYoutube />,
  },
  {
    link: "https://medium.com/@EXPANZ",
    icon: <FaMedium />,
  },
    ...
    <div>
      <Link
        className={`custom-link ${match && "text-primary"
          } grid grid-flow-col font-medium  text-base mb-2 justify-start gap-x-2 items-center py-2 px-6 hover:bg-primary-400 rounded-lg my-1 `}
        to={to}
        {...props}
      >
        {children}
