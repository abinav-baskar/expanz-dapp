...
const Account = ({ data }) => {
  ...
  const daily = "Daily ROI " + (new Intl.NumberFormat("en-US").format(config.DAILY)) + "%";
  ...
  const accountData = [
    {
      title: "Current XPANZ Price",
      result: "$" + formatNumber(data.marketPrice, 4) + " USD",
      color: "primary",
    },
    {
      title: "Next Reward Amount",
      result: (isNaN(data.balance) ? "0.00" : formatNumber(data.balance * 0.0003958125, 2)) + " XPANZ",
      color: "",
    },
    {
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="  grid grid-cols-1 gap-8 mb-10">
        <div className=" grid  grid-cols-1 sm:grid-cols-2 gap-x-6 gap-8 text-left">
          {/* <div className="p-6 bg-dark-400 rounded-xl bg-opacity-40 ">
            ...
            <h3 className="  text-xs uppercase text-white">
              {formatNumber(earnedTokenAmount(), 2)} XPANZ
          ...
          <div className="bg-dark-400 rounded-xl bg-opacity-40  p-6">
            <h3 className=" text-lightGray-400">APY</h3>
            <p className=" text-primary font-semibold text-5xl my-2">
              {apy}
            </p>
            <h3 className=" text-lightGray-400">{daily}</h3>
          </div>
          <div className="bg-dark-400 rounded-xl bg-opacity-40  p-6">
            <h3 className=" text-lightGray-400">Your Balance</h3>
            <p className=" text-primary font-semibold text-5xl my-2">
            {formatCurrency(data.balance * data.marketPrice, 2)}
            </p>
            <h3 className=" text-lightGray-400">{formatNumber(data.balance, 2)} XPANZ</h3>
          </div>
        </div>
        <div className=" grid  grid-cols-1 sm:grid-cols-2 gap-x-6 gap-8 text-left">
          {/* <div className="p-6 bg-dark-400 rounded-xl bg-opacity-40 ">
            ...
            <p className=" text-white ">{formatNumber(data.balance, 2)} XPANZ</p>
          </div> */}
          <div className="bg-dark-400 rounded-xl bg-opacity-40  p-6">
            <p className=" text-lightGray-400 text-sm">Rebase Time</p>
            <p className="text-2xl text-primary font-bold">{NextRebase()}</p>
          </div>
          <div className="bg-dark-400 rounded-xl bg-opacity-40  p-6">
            <p className=" text-lightGray-400 text-sm">
              Your Earnings/daily            </p>
            ...
            <p className=" text-lightGray-400 ">
              {isNaN(data.balance * config.DAILY) ?
                "0.00" :
                formatNumber(data.balance * config.DAILY / 100, 2)} XPANZ
            </p>
          </div>
          
        </div>

        <div className="bg-dark-400 rounded-xl bg-opacity-40 px-6 py-6 grid grid-cols-1 md:grid-cols-1 gap-x-20 gap-y-6">
          {accountData.map((val, i) => (
            <div
              className="flex justify-between items-center text-sm sm:text-base"
              key={i}
            >
              <p className=" text-lightGray-400 ">{val.title}</p>
