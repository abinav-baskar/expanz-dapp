...
const Calculator = ({ marketPrice = 0, titanoBalance = 0 }) => {
  ...
  const topList = [
    ...
    {
      title: "Your Balance",
      result: (titanoAmount || 0).toFixed(2) + " XPANZ",
    },
  ];
  ...
  const dataList = [
    ...
    },
    {
      ...
      result: "$" + calcCurrentWealth().toFixed(2),
    },
    {
      title: "XPANZ rewards estimation",
      result: rewardsEstimation.toFixed(2) + " XPANZ",
    },
  ...
  return (
    <div className="">
      <div className=" w-full mx-auto ">
        ...
        <h2 className="font-bold text-xl" style={{'color':'#999', 'fontSize':'1.5rem'}}>Calculator</h2>
        <h4 style={{'fontSize':'12px'}}>Estimate your returns</h4>
        <div className="my-8">
          <div className="grid md:grid-cols-3 gap-8 mb-8">
            {topList.map((val, i) => (
              <div
                key={i}
                className="px-4 py-6 md:p-6 md:px-8 rounded-xl bg-opacity-40 text-center"
              >
                <p className=" text-lightGray-400 text-xl">{i == 0 ? "" : val.title}</p>
                <h6>
                <span class="sc-kutgmK fHxhqk" style={{'color': 'rgba(253, 61, 86)', 'fontWeight':'bold','backgroundClip':'text', 'textShadow':'rgba(255, 48, 48, 0.94) 1px 0px 10px'}}>{i == 0 ? "XPANZ " : ""}</span> <span>{i == 0 ? "  price" : ""}</span>
          ...
          <div className=" grid md:grid-cols-2 mt-4 gap-10">
            <div>
              ...
              <span class="sc-kutgmK fHxhqk" style={{'color': 'rgba(253, 61, 86)', 'backgroundClip':'text', 'fontWeight':'bold', 'textShadow':'rgba(255, 48, 48, 0.94) 1px 0px 10px'}}>XPANZ</span> <span>Amount</span>
              </label>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-1 gap-x-20 gap-y-4">
            {dataList.map((val, i) => (
              <div
